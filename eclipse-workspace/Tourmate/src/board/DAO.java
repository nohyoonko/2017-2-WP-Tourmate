...
public ArrayList<VO> getMemberList() { 
	...
	try { 
		sql = "SELECT NUM, USERNAME, TITLE, TIME, HIT, INDENT from trip order by ref desc, step asc"; 
		pstmt = con.prepareStatement(sql); 
		rs = pstmt.executeQuery(); 
...
public int getMax() { 
	...
	}finally { DBClose.close(con,pstmt,rs); 
	} 
	return max; 
} 
...
public void insertWrite(VO vo, int max) { 
	...
	PreparedStatement pstmt = null; 
	try { 
		sql = "INSERT INTO trip(USERNAME,PASSWORD,TITLE,MEMO,REF) VALUES(?,?,?,?,?)"; 
		pstmt = con.prepareStatement(sql); 
...
public VO getView(int idx) { 
	...
	try { 
		sql = "SELECT USERNAME, TITLE, MEMO, TIME, HIT, PASSWORD, REF, INDENT, STEP FROM trip WHERE NUM=?"; 
		pstmt = con.prepareStatement(sql); 
		pstmt.setInt(1, idx); 
		rs = pstmt.executeQuery(); 
...
public void UpdateHit(int idx) { 
	...
	try { 
		sql = "UPDATE trip SET HIT=HIT+1 where NUM=?"; 
		pstmt = con.prepareStatement(sql); 
		pstmt.setInt(1, idx); 
		pstmt.executeUpdate(); 
	}catch(Exception e) { 
...
public boolean checkPassword(VO vo, int idx) { 
	...
	try { 
		sql = "SELECT NUM FROM trip where NUM=? and PASSWORD=?"; 
		pstmt = con.prepareStatement(sql); 
		pstmt.setInt(1, idx); 
...
public void delete(int idx) { 
	...
	try { 
		sql = "DELETE FROM trip WHERE NUM=?"; 
		pstmt = con.prepareStatement(sql); 
		pstmt.setInt(1, idx); 
		pstmt.executeUpdate(); 
	}catch(Exception e) { 
...
public void modify(VO vo, int idx) { 
	...
	PreparedStatement pstmt = null; 
	try { 
		sql = "UPDATE trip SET TITLE=?, MEMO=? where NUM=?"; 
		pstmt = con.prepareStatement(sql); 
...
public void UpdateStep(int ref, int step) { 
	...
	try { 
		sql = "UPDATE trip SET STEP=STEP+1 where REF=? and STEP>?"; 
		pstmt = con.prepareStatement(sql); 
		pstmt.setInt(1, ref); 
		pstmt.setInt(2, step); 
		pstmt.executeUpdate(); 
...
public void insertReply(VO vo, int ref, int indent, int step) { 
	...
	try { 
		sql = "INSERT INTO trip(USERNAME, PASSWORD, TITLE, MEMO, REF, INDENT, STEP) "+ "VALUES(?,?,?,?,?,?,?)"; 
		pstmt = con.prepareStatement(sql); 
		pstmt.setString(1, pasing(vo.getName())); 
		pstmt.setString(2, pasing(vo.getPassword())); 
		pstmt.setString(3, pasing(vo.getTitle())); 
